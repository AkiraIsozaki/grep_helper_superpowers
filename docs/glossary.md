...
| `間接（setter経由）` | setterメソッド経由で値が設定されている（Java / Groovy） |
...
| `ref_type` | `str` | 参照種別（`直接` / `間接` / `間接（getter経由）` / `間接（setter経由）`） |
| `usage_type` | `str` | 使用タイプ（言語ごとに 6〜7種のいずれか） |
...
### 4段階分析フロー（Java / Groovy）

**定義**: grep結果から文言の全使用箇所を洗い出す最大4段階の分析処理（Java / Groovy）

**説明**:
各段階は順序依存があり、前の段階が完了してから次の段階を実行する。言語によって実施される段階数が異なる（他言語は1〜2段階）。
...
| 第4段階 | setter経由追跡 | `SetterTracker`（第2段階のフィールド追跡後にのみ起動） | `間接（setter経由）` |
...
getter経由参照レコード（分類済み）
    ↓ (SetterTracker)   ← 第4段階（フィールド追跡後にのみ起動）
setter経由参照レコード（分類済み）
    ↓ (TsvWriter)
output/[文言].tsv
```
...
**使用例**: `python analyze.py --source-dir /path/to/java --input-dir /custom/input`
...
**使用例**: `python analyze.py --source-dir /path/to/java --output-dir /custom/output`
...
**発生条件**: ソースファイル読み込み時のデコードエラー（全言語共通。例: Shift-JIS / cp932 / UTF-8 などの混在環境）
