...
| 第2段階 | 第1段階で「定数定義」または「変数への代入」と判定された行を起点に、その定数・変数が使われている別の箇所を追跡して追加出力する | Java / C / Pro\*C / Kotlin / C# / VB.NET / Groovy（プロジェクト全体または同一クラス/メソッド内）、Shell / SQL（同一ファイル内のみ） |
...
**Java（`analyze.py` → `grep_helper.languages.java`）の場合**:
...
**その他の言語（C / Pro\*C / Kotlin / PL/SQL / SQL / Shell / TypeScript / Python / Perl / C# / VB.NET / Groovy）の場合** （`grep_helper.languages.<lang>.classify_usage`）:
...
### 9-2. Java アナライザー（`analyze.py` → `grep_helper.languages.java`）の呼び出し詳細
...
| 言語 | CLI スクリプト（shim） | ハンドラモジュール | 第1段階（分類） | 第2段階（間接） | 第3段階（getter） | 第4段階（setter） |
|------|------------|-----------|:---:|:---:|:---:|:---:|
| Java | `analyze.py` | `grep_helper.languages.java` | ✅ AST + 正規表現 | ✅ 定数・フィールド・ローカル変数 | ✅ | ✅ |
| C | `analyze_c.py` | `grep_helper.languages.c` | ✅ 正規表現 | ✅ #define / 変数 | — | — |
| Pro\*C | `analyze_proc.py` | `grep_helper.languages.proc` | ✅ 正規表現 | ✅ #define / 変数 | — | — |
...
この重複読み込みを回避するため、全言語ハンドラは「一度読み込んだファイルの内容をメモリ上に保持する（キャッシュする）」仕組みを共有している（`grep_helper.file_cache` モジュールの `cached_file_lines`）。キャッシュはサイズベースの LRU 方式で、合計バイト数の上限はデフォルト 256MB。上限を超えると最も古くアクセスされていないエントリから順に破棄される。

加えて、定数・getter・setter のように「同じ名前のリストをプロジェクト全体で照合する」処理では、複数の名前を1パスで一括検出するバッチスキャナを使う（`grep_helper.scanner.build_batch_scanner`）。名前の数が多い場合（しきい値: 100件以上）は Aho-Corasick アルゴリズム（`pyahocorasick` パッケージが利用可能ならそれを、無ければ `grep_helper._aho_corasick` の純Python実装にフォールバック）に自動的に切り替わり、件数に対してほぼ線形の処理時間で走査できる。
...
ファイル拡張子（または拡張子なしファイルのシバン行）で言語を判定して適切な分類器に振り分ける（内部実装: `grep_helper.dispatcher` / `grep_helper.languages.detect_handler`）。
対応外拡張子（.xml / .yaml / .properties など）は使用タイプ「その他」として出力され、漏れゼロを保証する。
出力は `output/TARGET.tsv` 1本（既存アナライザーと同形式）。

間接追跡対応状況（analyze_all.py 経由）:
- Java: プロジェクト全体（定数・フィールド・ローカル変数）、getter/setter経由
- Kotlin/C#・VB.NET: プロジェクト全体の定数追跡
- C/Pro*C: プロジェクト全体の #define 定数追跡
