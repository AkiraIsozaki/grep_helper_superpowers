...
dispatcher.main()
    ↓  build_parser() でオプション解析（--no-mmap / GREP_HELPER_NO_MMAP=1 は _resolve_use_mmap で解決、
       --handler-workers N で Phase 2 のハンドラ並列度を指定）
    ...
    Phase 2 + インクリメンタル書き出し:
        集約した直接参照を apply_indirect_tracking(..., handler_workers, on_handler_complete)
        に渡し、ProcessPool (max_workers=handler_workers) で各ハンドラの batch_track_indirect
        を並列実行（handler_workers <= 1 のとき直列、ProcessPool 構築失敗時も直列にフォールバック）。
        on_handler_complete コールバックが handler 完了ごとに呼ばれ、
        pending[kw] が空になった keyword の TSV を即時 write_tsv で書き出す
        （旧 Phase 3 のまとめ書き出しは廃止）。
    ドレインフォールバック: 1 handler 全失敗等で pending が残った keyword は最後に書き出す。
    ↓  処理サマリを stdout に表示
```

**1 handler 例外時の挙動**: 子プロセス内の例外（`ImportError` / `AttributeError` / `batch_track_indirect`
内の logic 例外を含む）は `fut.result()` の try/except で吸収され、stderr 警告を出して当該 handler は
スキップ、他 handler の結果は通常通り集約・書き出しされる。これにより 1 handler の障害が全体を
失敗させない（spec 2026-05-11 §エラー処理 参照）。

`on_handler_complete` は親プロセスのメインスレッドから `as_completed` の同期ループ内でのみ
呼ばれる契約のため、`pending` / `indirect_by_keyword` / `written` の更新はロックなしで
thread-safe である。
...
| `_filter_byte_cache` | `grep_helper.source_files`（`grep_filter_files` のファイル単位 byte hit cache） |
| `_encoding_cache` | `grep_helper.encoding` |
...
| `grep_filter_files` | `(names, src_dir, extensions, label="", *, use_mmap=True)` → `list[Path]` | mmap バイト前フィルタ。`_filter_byte_cache: dict[(str(path), bytes), bool]` により同一 (path, pattern) の 2 回目以降は I/O ゼロで返す。OSError 発生時のみ safe-side True を返すが cache には書かない（transient エラーの誤永続化を防ぐ） |
| `build_batch_scanner` | `(patterns)` → `_BatchScanner` | パターン数 ≥ 100 で `pyahocorasick`（Aho-Corasick 法）を自動選択 |

---

## 並列処理（`--workers` / `--handler-workers`）

`analyze_all.py`（`dispatcher.main`）と各個別アナライザーは `--workers N`（既定: 1）を受け付け、各ハンドラ内部の `batch_track_indirect` のバッチスキャン処理を `ProcessPoolExecutor` で並列化する。各ワーカーは独立プロセスで動作するため GIL の制約を受けない。
...
加えて `analyze_all.py`（`dispatcher.main`）専用に **`--handler-workers H`（既定: 2）** が用意されており、Phase 2 でハンドラ単位を並列実行する。各ハンドラは独立プロセスで動的 import → `batch_track_indirect` を呼び出し、戻り値は `as_completed` で逐次受け取られて `on_handler_complete` 経由で TSV にインクリメンタル書き出しされる。Java と他言語が混在する grep セットで効果が大きい。

```bash
# handler 並列 (H=2) × 内側 workers (N=2) で計 4 プロセス
python analyze_all.py --source-dir ./src --workers 2 --handler-workers 2
```

階層は **`H × N` 個のプロセス**になるため、Solaris 4〜8 コア環境では `H=2, N=2` が標準目安。Linux で CPU が十分なら `H=3〜4, N=2〜4` まで試す余地がある。

**メモリの目安**:
- ファイル行キャッシュ: 256MB × `N` ワーカー（既存挙動）
- `_filter_byte_cache`: ~40MB / ワーカー（5,000 ファイル × 100 パターン × ~80B、実測値）
- handler 並列の場合、`H × N` プロセス分の上記キャッシュが独立して存在する（プロセス間共有はしない）

**ProcessPool 構築失敗時の挙動**: `OSError`（Solaris fork 失敗、`ulimit -u` 不足等）を catch して `handler_workers=1` の直列実行に自動フォールバックする。`stderr` に警告は出すが処理は継続する。

**1 handler 例外時の挙動**: 子プロセス内例外は `fut.result()` の try/except で吸収。他 handler の indirect は無事に集約されて TSV に書き出される。詳細は spec `docs/superpowers/specs/2026-05-11-perf-handler-parallel-design.md` §エラー処理 参照。
