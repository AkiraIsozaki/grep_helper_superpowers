...
├── analyze_common.py    # 全言語共通インフラ（GrepRecord, ProcessStats, parse_grep_line, write_tsv, detect_encoding, ファイル/ソース/解決パスの各種キャッシュ, build_batch_scanner, grep_filter_files）
...
├── aho_corasick.py      # Pure Python Aho-Corasick（オフライン環境フォールバック用。pyahocorasick が無い場合に build_batch_scanner から使用）
├── tests/test_analyze.py          # Javaアナライザーのユニットテスト・統合テスト
├── tests/test_analyze_proc.py     # Pro*Cアナライザーのユニットテスト・統合テスト
├── tests/test_aho_corasick.py     # Pure Python Aho-Corasickのユニットテスト
...
│   │   ├── src/
│   │   └── expected/
│   ├── all/             # 全言語ディスパッチャー（test_all_analyzer.py用。expected/ なし）
│   │   ├── input/
│   │   └── src/
...
- `cached_file_lines()`: ファイル行リストのサイズベース LRU キャッシュ（既定 256MB 上限）
- `set_file_lines_cache_limit()`: 上記キャッシュの上限バイト数を変更
- `resolve_file_cached()`: ファイルパス解決（CWD 相対 → src_dir 相対）の結果キャッシュ
...
- `test_aho_corasick.py`: Pure Python Aho-Corasick (`aho_corasick.py`) のユニットテスト
...
| 全言語ディスパッチャー | プロジェクトルート | `analyze_all.py` | - |
| 補助ライブラリ（Pure Python フォールバック） | プロジェクトルート | `aho_corasick.py` | - |
...
| 補助ライブラリのテスト | `tests/` | `test_[対象モジュール].py` | `tests/test_aho_corasick.py` |
...
analyze_common.py   ← 共通インフラ（全言語から依存可）
    ↓ (import)
re, csv, argparse, pathlib, sys, dataclasses, enum, heapq, tempfile, mmap, collections.OrderedDict  # 標準ライブラリ
chardet         # オプション（未インストール時は cp932 フォールバック）
pyahocorasick   # オプション（未インストール時は同梱の aho_corasick.py にフォールバック）
aho_corasick.py # pyahocorasick が無い場合の Pure Python フォールバック実装
...
analyze_all.py
    ↓ (import)
analyze_common  # 全言語ディスパッチャー（拡張子/シバン判定）
```
...
```
.claude/
├── skills/                  # タスクモード別スキル定義（例: coding-conventions.md）
└── settings.local.json      # Claude Code設定ファイル（ローカル）
```

### CLAUDE.md（AIアシスタント設定）

**役割**: Claude Codeへの動作指示とプロジェクトメモリを置くためのファイル。技術スタック・開発プロセス・ディレクトリ構造の概要を必要に応じて記載する（現状は空ファイル）。
...
- `htmlcov/`・`.coverage`（カバレッジ計測の生成物）
- `.ai/`・`.idea/`（エディタ・AIツール設定）
- `.worktrees/`（作業用 git worktree 配置先）
