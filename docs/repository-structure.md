...
├── analyze_common.py    # 全言語共通インフラ（GrepRecord, ProcessStats, parse_grep_line, write_tsv）
...
| 言語別アナライザー | プロジェクトルート | `analyze_[言語].py` | `analyze_c.py`, `analyze_proc.py` |
...
| C/SQL/Shellアナライザーテスト | `tests/` | `test_[言語]_analyzer.py` | `test_c_analyzer.py` |
...
test_analyze.py / test_analyze_proc.py / tests/test_*.py
    ↓ (import)
analyze.py / analyze_c.py / analyze_proc.py / analyze_sh.py / analyze_sql.py
    ↓ (import)
...
**禁止される依存**:
- `analyze_common.py` に外部ライブラリを追加しない（javalangも不可）
- `analyze.py` 以外のアナライザーに `javalang` を追加しない
- テストファイルに `unittest` 以外の外部ライブラリを追加しない
