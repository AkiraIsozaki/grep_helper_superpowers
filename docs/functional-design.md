...
- 全言語アナライザー（`analyze_c.py`, `analyze_proc.py`, `analyze_sh.py`, `analyze_sql.py`, `analyze_kotlin.py`, `analyze_plsql.py`, `analyze_ts.py`, `analyze_python.py`, `analyze_perl.py`, `analyze_dotnet.py`, `analyze_groovy.py`）がそれぞれモジュールレベルで保持する。`analyze.py`（Java）は `_file_lines_cache` として保持する
