...
graph TB
    User[調査担当者]
    CLI["CLIレイヤー / analyze.py（shim → grep_helper.cli.run）"]
    Stage1[第1段階: 直接参照分類]
    Stage2[第2段階: 間接参照追跡]
    Stage3[第3段階: getter経由追跡]
    Stage4[第4段階: setter経由追跡]
    Output["TSV出力 / grep_helper.tsv_output.write_tsv()"]
...
| AST解析 | javalang（`grep_helper.languages.java*` のみ） | Java 7以上のソースをPythonからAST解析できる唯一の実績あるライブラリ |
| 言語別分類 | re（標準ライブラリ） | Java 以外（C/Pro*C/SQL/Shell/Kotlin/PL/SQL/TS/Python/Perl/.NET/Groovy）はすべて正規表現のみで分類完結。外部依存不要 |
| 文字コード推定 | chardet（`requirements.txt` 必須） | grep 結果 / ソースファイルの文字コード自動検出（`grep_helper.encoding.detect_encoding`）。万一未インストールなら cp932 フォールバック |
| 多パターン検索 | pyahocorasick（`requirements.txt` 必須） + 同梱純Python実装 | 一括追跡時のパターン数 ≥ 100 で Aho-Corasick に自動切替（`grep_helper.scanner.build_batch_scanner`）。万一未インストールなら同梱の `grep_helper._aho_corasick` 純Python実装にフォールバック |
| TSV出力 | csv（標準ライブラリ） | タブ区切り・BOM付きUTF-8出力をネイティブサポート（`grep_helper.tsv_output`） |
| 大規模ソート | heapq（標準ライブラリ） | 100万件超のレコードをチャンク分割+ヒープマージで外部ソート（`grep_helper.tsv_output`） |
| CLIパース | argparse（標準ライブラリ） | --source-dir等のオプション解析に十分（`grep_helper.cli`） |
...
# grep_helper.pipeline
def process_grep_file(path: Path, keyword: str, handler, source_dir: Path, stats: ProcessStats,
                      *, encoding_override: str | None = None) -> list[GrepRecord]:
    """grepファイル全行を処理し、第1段階（直接参照）レコードのリストを返す。
    grep結果ファイル / ソースファイルともに `grep_helper.encoding.detect_encoding()` で
...
**依存関係**: `re`（標準ライブラリ）, `grep_helper.model.ProcessStats`, `grep_helper.encoding.detect_encoding`, `grep_helper.grep_input.iter_grep_lines`
...
**依存関係**: `javalang`, `re`, `grep_helper.model.ProcessStats`, `grep_helper.languages.java_ast`（ASTキャッシュ）
...
**依存関係**: `grep_helper.languages.java_classify`, `grep_helper.languages.java_ast`, `javalang`
...
- プロセス全体で共有するモジュールレベルのdict（`grep_helper.languages.java_ast` 内のみ）

```python
# grep_helper.languages.java_ast に定義。
...
**責務**:
- 各言語ハンドラで同一ファイルの繰り返し読み込みを省略する
- 実体は `grep_helper.file_cache` の `_file_lines_cache`（プロセス内シングルトン OrderedDict）。
  全言語ハンドラは `cached_file_lines(path, encoding, stats)` 経由で同一キャッシュを共有する
- 行数ではなくおおよそのバイト数（行長 + 64バイト/行のオーバーヘッド）で見積もり、
  バイト上限を超過したら最古エントリから破棄する LRU 動作

```python
# grep_helper.file_cache 内
...
### grep_filter_files（`grep_helper.source_files`）
...
**呼び出し元**:  
- `grep_helper.languages.java_track`: `batch_track_combined`（定数 / getter / setter を 1 パスで統合追跡）
- 各言語ハンドラの `batch_track_indirect`（Kotlin / .NET / Groovy / C / Pro*C 等）

**エラー時**: OSError / mmap.error / 空ファイルが発生したファイルはスキャン対象に含める（安全側フォールバック）

---

### F-07: 言語別ハンドラ（Java 以外の全言語）

**責務（共通）**:
- grep行を `grep_helper.grep_input.parse_grep_line()` でパースする（パイプラインから呼ばれる）
- 言語固有の正規表現パターンで使用タイプを分類する（`classify_usage(code, *, ctx)` として公開）
- 間接追跡が必要な言語は `batch_track_indirect(direct_records, src_dir, encoding, *, workers)` を実装する
- 結果を `GrepRecord` リストとして返し `grep_helper.tsv_output.write_tsv()` で出力する

**言語別使用タイプ**:

| 言語 | モジュール | 使用タイプ（種数） |
|------|-----------|-----------------|
| C | `grep_helper.languages.c` | #define定数定義・条件判定・return文・変数代入・関数引数・その他（6種） |
| Pro*C | `grep_helper.languages.proc` | EXEC SQL文・#define定数定義・条件判定・return文・変数代入・関数引数・その他（7種） |
| Oracle SQL | `grep_helper.languages.sql` | 例外・エラー処理・定数・変数定義・WHERE条件・比較・DECODE・INSERT/UPDATE値・SELECT/INTO・その他（7種） |
| Shell | `grep_helper.languages.sh` | 環境変数エクスポート・変数代入・条件判定・echo/print出力・コマンド引数・その他（6種） |
| Kotlin | `grep_helper.languages.kotlin` | const定数定義・変数代入・条件判定・return文・アノテーション・関数引数・その他（7種） |
| PL/SQL | `grep_helper.languages.plsql` | 定数/変数宣言・EXCEPTION処理・条件判定・カーソル定義・INSERT/UPDATE値・WHERE条件・その他（7種） |
| TypeScript/JavaScript | `grep_helper.languages.ts` | const定数定義・変数代入(let/var)・条件判定・return文・デコレータ・関数引数・その他（7種） |
| Python | `grep_helper.languages.python` | 変数代入・条件判定・return文・デコレータ・関数引数・その他（6種） |
| Perl | `grep_helper.languages.perl` | use constant定義・変数代入・条件判定・print/say出力・関数引数・その他（6種） |
| C#/VB.NET | `grep_helper.languages.dotnet` | 定数定義(Const/readonly)・変数代入・条件判定・return文・属性(Attribute)・メソッド引数・その他（7種） |
| Groovy | `grep_helper.languages.groovy` | static final定数定義・変数代入・条件判定・return文・アノテーション・メソッド引数・その他（7種） |

**Pro*C 固有機能（拡張子ベースのディスパッチ）**:
```python
# grep_helper.languages.proc.classify_usage
def classify_usage(code: str, *, ctx: ClassifyContext | None = None) -> str:
    ext = Path(ctx.filepath).suffix.lower() if ctx else ""
    if ext in ('.c', '.h'):
        return c.classify_usage(code)   # grep_helper.languages.c から委譲
    return _classify_usage_proc(code)   # .pc ファイル向け Pro*C 分類
```

**間接参照追跡（C/Pro*C/Shell/SQL/Kotlin/C#・VB.NET/Groovy）**:
- C/Pro*C: `#define定数定義` → プロジェクト全体の `.c`/`.h`/`.pc` ファイルを追跡（`batch_track_indirect`）
- Shell: `変数代入` / `環境変数エクスポート` → 同一ファイル内を追跡（`batch_track_indirect`）
- SQL: `定数・変数定義` → 同一ファイル内を追跡（`batch_track_indirect`）
- Kotlin: `const定数定義` → プロジェクト全体の `.kt`/`.kts` ファイルを追跡（`batch_track_indirect`）
- C#/VB.NET: `const` / `static readonly` 定数定義 → プロジェクト全体の `.cs`/`.vb` ファイルを追跡（`batch_track_indirect`）
- Groovy: `static final` 定数・フィールド → プロジェクト全体の `.groovy`/`.gvy` ファイルを追跡（`batch_track_indirect`）
- Groovy: getter/setter経由の呼び出し箇所追跡（`batch_track_indirect` 内で実行）
...
- **ASTキャッシュ**: `grep_helper.languages.java_ast._ast_cache` でファイルごとにキャッシュ。同一ファイルの再解析を完全省略
- **AST 行インデックス**: `grep_helper.languages.java_ast._ast_line_index` で `(usage_type, scope)` を O(1) 参照
- **ファイル行キャッシュ**: `grep_helper.file_cache._file_lines_cache`（バイト数ベース LRU、上限 256MB）を全言語で共有
- **ソースファイル一覧キャッシュ**: `grep_helper.source_files.iter_source_files` が `(src_dir, extensions)` 単位で `rglob` 結果を再利用
- **mmap 事前フィルタ**: `grep_helper.source_files.grep_filter_files()` でバッチスキャン前に対象ファイルをバイト列検索で絞り込む
- **バッチスキャナ**: `grep_helper.scanner.build_batch_scanner(patterns, threshold=100)` がパターン数に応じて
  combined regex / Aho-Corasick（`pyahocorasick` または `grep_helper._aho_corasick.AhoCorasick` の純 Python 実装）を自動選択
- **並列スキャン**: `--workers >= 2` で `grep_helper.languages.java_track.batch_track_combined` を `ProcessPoolExecutor` 並列化
- **ジェネレータ**: 大量のgrep行をメモリ効率良く処理（`grep_helper.grep_input.iter_grep_lines` でストリーミング）
- **プリコンパイル正規表現**: `USAGE_PATTERNS` は起動時に一度コンパイル（モジュールレベル定数）
- **外部ソート**: `grep_helper.tsv_output.write_tsv` は 100万件超で `heapq.merge` ベースの外部ソートに切り替え
...
|-------------|------------|------------|
| `tests/test_analyze.py` | `grep_helper.languages.java` | `classify_usage()`, `classify_usage_regex()`, `batch_track_indirect()` 等 |
| `tests/test_analyze_proc.py` | `grep_helper.languages.proc` | `classify_usage()`, `batch_track_indirect()`, Pro*C E2Eフロー |
| `tests/test_common.py` | `grep_helper.model` / `grep_helper.grep_input` / `grep_helper.tsv_output` | `parse_grep_line()`, `write_tsv()`, 共通データモデル |
| `tests/test_c_analyzer.py` | `grep_helper.languages.c` | C E2Eフロー（直接参照 + #define間接参照） |
| `tests/test_sh_analyzer.py` | `grep_helper.languages.sh` | Shell E2Eフロー |
| `tests/test_sql_analyzer.py` | `grep_helper.languages.sql` | SQL E2Eフロー |
| `tests/test_kotlin_analyzer.py` | `grep_helper.languages.kotlin` | Kotlin E2Eフロー（直接参照 + const val間接参照） |
| `tests/test_plsql_analyzer.py` | `grep_helper.languages.plsql` | PL/SQL E2Eフロー（直接参照） |
| `tests/test_ts_analyzer.py` | `grep_helper.languages.ts` | TypeScript/JS E2Eフロー（直接参照） |
| `tests/test_python_analyzer.py` | `grep_helper.languages.python` | Python E2Eフロー（直接参照） |
| `tests/test_perl_analyzer.py` | `grep_helper.languages.perl` | Perl E2Eフロー（直接参照） |
| `tests/test_dotnet_analyzer.py` | `grep_helper.languages.dotnet` | C#/VB.NET E2Eフロー（直接参照 + const/static readonly間接参照） |
| `tests/test_groovy_analyzer.py` | `grep_helper.languages.groovy` | Groovy E2Eフロー（直接参照 + static final間接参照 + setter追跡） |
| `tests/test_aho_corasick.py` | `grep_helper._aho_corasick` | AhoCorasick の単語境界マッチ・複数パターン同時検索 |
| `tests/test_all_analyzer.py` | `grep_helper.dispatcher` | 多言語ディスパッチ・統合追跡パイプラインの E2E |
