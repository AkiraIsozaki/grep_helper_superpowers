...
| 配布 | venv + zip + Makefile | 解凍→setup.sh→run.shで即使用可能 |
...
### FileCache（C/Pro*C/SQL/Shell 共通ファイル行キャッシュ）

**責務**:
- 各言語アナライザーで同一ファイルの繰り返し読み込みを省略する
- ASTキャッシュを持たない言語アナライザー（`analyze_c.py`, `analyze_proc.py`, `analyze_sh.py`, `analyze_sql.py`）がそれぞれモジュールレベルで保持する
