...
### L. 自明な method docstring 削除 (pattern F、Wave 1 で確立)

**目的:** メソッド名が日本語で WHAT を明示している場合、`def test_xxx(self):` 直下の 1 行 docstring はそれを言い直しているだけで冗長。削除して spec-as-collection 原則を強化する。

**Wave 1 前例:** commit `df3965b` で test_analyze.py の 99 件を機械削除済。Wave 2 でも同方針を踏襲する。

**削除トリガー (削除する):**
- メソッド docstring が method 名 + 「こと」「されること」を付け足しただけ:
  ```python
  def test_export文を環境変数エクスポートとして分類する(self):
      """export 構文が環境変数エクスポートに分類されることを検証"""
  ```
  → docstring 削除（method 名で十分）
- 単に「<method 名> をテストする」「<method 名> の動作を検証する」型の言い換え
- WHAT を別表現で書いただけで情報量ゼロのもの

**保持 (削除しない):**
- docstring に method 名にない情報がある (制約条件、edge case の前提、特殊事情の注釈、他テストとの差分説明)
- パイロット G スパイク所見等の出典 reference を含む
- skip 条件や fixture 前提条件を補足説明している
- 段落形式の長い解説（複数行）

**判定の手順:**
1. `def test_xxx(self):` 直後の docstring を確認。
2. method 名の単語と docstring の単語が **同じ概念のみ** を別表現で書いていれば削除。method 名にない事実が docstring にあれば保持。
3. 迷ったら保持（false positive 削除より false negative 保持の方が安全）。

**コミット粒度:** 1 ファイル 1 commit。判定アクション (c/b/a) や method-name 改名 (K) とは別の独立コミット。

**コミットメッセージ:**

```
test(<file>): drop redundant method docstrings (pattern F)

メソッド名が日本語で WHAT を表現しているため docstring は冗長。
N 件削除（method 名にない情報を含む M 件は保持）。

Wave 1 commit df3965b と同方針。method 本体は不変、テスト件数も不変。
...
- [ ] **Step 5c: 自明な method docstring 削除 (pattern F)** (`共通の事前確認事項 L` に従う)

method 名が WHAT を明示しているのに docstring がそれを言い直しているだけのものを削除。`def test_xxx(self):` 直下の 1 行 docstring が対象。method 本体は不変、別 commit。該当 0 件なら commit 不要。
