...
- Task 1 (commit `6b6a0d7`): 決定的ソート完全化（`_sort_key` 5 タプル化）
- Task 2 (commit `fd74cb0` + fix `f7665dc`): file-level byte hit cache（`grep_filter_files` API 不変）
- Task 3 (commit `c917c97`): handler 並列化インフラ（`apply_indirect_tracking` に `handler_workers` / `on_handler_complete` 追加）
- Task 4 (commit `a8f56bb` + lint fix `89ddff3`): インクリメンタル TSV 出力 + CLI `--handler-workers`

### V-1 検証結果（dev コンテナ Linux x86_64 / Python 3.12.13）

ベースライン: `75a8a9e` (plan commit, before implementation)。検証 HEAD: `89ddff3` (Task 4 + lint fix 完了地点)。

#### 1. KPI 回帰チェック（pipeline 経由）

`scripts/measure_kpi.py --lang all --quiet` の出力を before/after で比較:
...
19 行すべて完全一致。網羅率・分類精度の回帰なし（pipeline は無変更なので想定通り）。

#### 2. 性能計測（dispatcher 経由、`tests/golden/java/`）

`time` の `real` 値を 3 回試行で記録。golden fixture は src 21 ファイル、`.grep` 入力 2 件（777.grep / CODE.grep）と小規模。

| 構成 | run1 | run2 | run3 | 中央値 |
|---|---|---|---|---|
| BEFORE (`--workers 2`) | 0.805 s | 0.861 s | 0.744 s | **0.805 s** |
| AFTER (`--workers 2 --handler-workers 2`) | 0.803 s | 0.835 s | 0.821 s | **0.821 s** |
| AFTER (`--workers 2 --handler-workers 4`) | 0.775 s | 0.510 s | 0.544 s | **0.544 s** |

- handler-workers=2 では BEFORE と有意差なし（0.805 s → 0.821 s）。java 言語の indirect tracking ハンドラ数（getter / setter / annotation 等）に対して並列度 2 だと並列効果がほぼ出ない、または fixture 規模が小さく Python の thread 起動オーバーヘッドに埋もれている。
- handler-workers=4 では 0.805 s → 0.544 s（中央値で **約 32% 短縮**）。並列度を上げると効果が出るため、機構自体は機能している。
- 注: 単一 `.grep` あたり 20 行程度の出力、wall clock ~0.5–0.8 s と fixture が小さく、ノイズに対する S/N 比は限定的。Solaris 実機・本番規模では handler 並列の効果がより明確に現れると期待する。

TTFB（最初の `→` 行までの時間）はログから 1 行目までの差分として観測可能だが、wall clock 全体に対するノイズが大きく信頼区間を取れる規模ではないため、ここでは記録省略。Solaris 実機計測時に再評価する。

#### 3. TSV 行セット一致

`/tmp/out_before/*.tsv` と `/tmp/out_after/*.tsv` を `diff -q` で比較:

| ファイル | 行数 | バイト数 | 結果 |
|---|---|---|---|
| `777.tsv` | 509 | 42087 | バイト一致 |
| `CODE.tsv` | 8 | 782 | バイト一致 |

決定的ソート（Task 1）と handler 並列化（Task 3-4）を経ても出力は完全に同一。並列実行による insertion-order 依存 tie は混入していない。

#### 4. メモリ使用量

`/usr/bin/time -v` が dev コンテナに無いため、Python の `resource.getrusage(RUSAGE_CHILDREN)` で `ru_maxrss` を計測:

| 構成 | wall clock | Peak RSS |
|---|---|---|
| BEFORE (`--workers 2`) | 0.975 s | **63.3 MB** |
| AFTER (`--workers 2 --handler-workers 2`) | 0.582 s | **65.1 MB** |

差分 +1.8 MB（B2' file-level byte hit cache + handler 並列化のスレッドスタック / ローカルバッファ）。1 GB 予算に対して 0.07% 未満で、想定範囲内。

### 総括

- 正確性（KPI / TSV）は完全に保全。
- 性能は dev コンテナの小規模 fixture では `--handler-workers 4` で約 32% 短縮を確認。`--handler-workers 2`（デフォルト値）では fixture が小さすぎて誤差レベル。
- メモリ予算 1 GB に対し実測 ~65 MB で大幅に余裕あり。
- 本番規模・Solaris 実機での効果検証は `scripts/smoke_solaris.md` の手順で別途実施する。
