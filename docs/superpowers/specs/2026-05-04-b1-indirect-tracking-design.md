...
- `tests/test_python_analyzer.py`（既存ファイルへ追記）
- `tests/test_ts_analyzer.py`（既存ファイルへ追記）
- `tests/test_perl_analyzer.py`（既存ファイルへ追記）
- `tests/test_plsql_analyzer.py`（既存ファイルへ追記）
...
#### 起点フィルタを ALL_CAPS のみに絞る理由

`zero-indent` を OR 条件に入れると、`app = Flask(__name__)` / `db = SQLAlchemy()` のような実プロジェクト典型のシングルトン代入を全部起点として登録してしまう。`\bapp\b` のような短い名前で全コードベースを scan すると false positive が爆発するため、**Python の慣習的なモジュール定数規約（ALL_CAPS）に絞る** ことで実用的な精度を確保する。命名規約に従わない定数は KPI で取りこぼしが顕在化したら個別対応とする（後続タスク扱い）。
...
#### エッジケース・スコープ外
- 小文字命名のシングルトン（`app = Flask()` / `db = SQLAlchemy()` 等）: 起点に**含めない**（`isupper()` で除外）
- `__all__ = [...]` 等の dunder 名: `__all__`.isupper() は False（cased letter 無し）なので起点には**ならない**（=妥当）
...
#### handler 内での Perl 2 系統の扱い（固定方針）

`extract_*` が2系統あり、それぞれ使用箇所スキャンパターンが異なる（定数 = `\bFOO\b` / `our $FOO` = `\$FOO\b`）ため、`build_batch_scanner` を共有できない。よって `batch_track_indirect` 内で **2種類のタスク辞書を作り、`_batch_track_perl_constant` を 2 回呼ぶ** 構造で固定する。

```python
def batch_track_indirect(direct_records, src_dir, encoding, *, workers=1):
    constant_tasks: dict[str, list[GrepRecord]] = {}   # use constant FOO 系統
    our_var_tasks: dict[str, list[GrepRecord]] = {}     # our $FOO 系統
    # ... 起点フィルタで2つの dict に振り分け ...
    results = []
    if constant_tasks:
        results.extend(_batch_track_perl_constant(constant_tasks, src_dir, ..., kind="bareword"))
    if our_var_tasks:
        results.extend(_batch_track_perl_constant(our_var_tasks, src_dir, ..., kind="scalar"))
    return results
```

`_batch_track_perl_constant` には `kind` パラメータ（`"bareword"` または `"scalar"`）を持たせ、内部で検索パターンを切り替える。
...
- **同一行マルチステートメント** `subtype t IS NUMBER; c_x CONSTANT t := 1;` のような 1 行に複数宣言を書くスタイル: `^\s*(\w+)\s+CONSTANT` は行頭からのマッチのため**取りこぼす**。`sql.py` の既存 `_SQL_VAR_PATTERN` も同様の制約。実プロジェクトでは1行1宣言が支配的なためスコープ外として許容
...
既存の `tests/test_python_analyzer.py` / `test_ts_analyzer.py` / `test_perl_analyzer.py` / `test_plsql_analyzer.py` に、新規クラス（例: `TestExtractModuleConstName` / `TestBatchTrackIndirect` 等）を追記する形で実装する。新規ファイルは作らない（既存と命名規約・対象モジュールが重複するため）。
...
| `batch_track_indirect` の workers=2 | **公開 API 経由のブラックボックス**で `workers=2` の結果が `workers=1` と一致することのみ検証（`ProcessPoolExecutor.submit` を直接呼ばない） | `test_並列実行でも直列と同じレコード集合を返す` |
| 定義行の自己参照除外 | 定義行自体が間接レコードにならないこと（`kotlin.py` L65-68 と同様） | `test_定義行自身は間接レコードに含まれない` |

**並列テストの環境前提**: `ProcessPoolExecutor` のテストは **Linux fork 前提**で書く（本リポジトリの dev 環境は Linux devcontainer）。macOS / Windows の spawn 環境での動作はスコープ外。spawn 環境でも `_scan_files_for_*_*` がモジュールトップレベル関数なら原理的に動くはずだが、検証はしない。
...
- `tests/test_python_analyzer.py` に新規クラスを追記
...
| 必須変更 | `grep_helper/languages/perl.py` | 26 行 → 約 270 行（Tier 1 + Tier 2 ハッシュ形式 + 2 系統呼び分け）|
...
| 必須変更 | `tests/test_python_analyzer.py` | 新規クラス追記（既存 `TestClassifyUsagePython` 等の隣に `TestExtractModuleConstName` / `TestBatchTrackIndirect` 等を追加）|
| 必須変更 | `tests/test_ts_analyzer.py` | 同上 |
| 必須変更 | `tests/test_perl_analyzer.py` | 同上 |
| 必須変更 | `tests/test_plsql_analyzer.py` | 同上 |
