...
| 11 | 635 | TestGetMethodScope | `_get_method_scope` (private, `tests/test_analyze.py:37`) | b 案 | パイロット G スパイク所見（`docs/superpowers/specs/2026-05-03-test-style-rework-design.md` 末尾）参照。`grep_helper/languages/java.py:119, 309` から直接呼ばれる。`TestIntenseE2E` (`tests/test_analyze.py:1361-1392`) で track_local 経路が踏まれる**はず**だった。**Phase 2 Task 7 mutation 実測:** 戻り値を `(1, 99999)` / `(method_start+1000, i)` / `(method_start, method_start)` の 3 通りに歪めても TestIntenseE2E は全件 OK（TestGetMethodScope 自身のみ赤化）。`_search_in_lines` の正規表現フィルタが過大／縮退いずれの scope でも観察不能化を吸収するため、b 案前提不成立。**判定上書き:** b 案 → c 案 fallback、`TestGetMethodScopeWhitebox` で保持（commit `859a84d`）。 | ~~クラス全削除前に E2E mutation 確認~~ → b 案前提不成立のため c 案 fallback で `TestGetMethodScopeWhitebox` Whitebox 化（Task 7 で実施済） |
