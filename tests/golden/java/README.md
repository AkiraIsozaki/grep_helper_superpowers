...
## 状態
- 本番規模: 各使用タイプ ≥10件、合計 500件超（actual 511 件）
...
| 条件判定 | 67 | Service.java, BigService.java, Handler.java, Demo.java |
...
| メソッド引数 | 134 | Caller.java, BigCaller.java, MoreCallers.java, Demo.java |
...
|---|---|---|
| 直接 | 434 | 全サンプルファイル |
| 間接（定数経由 / フィールド経由 / ローカル変数経由） | 71 | Constants.CODE → Service / Demo, Entity.type フィールド, Handler/Mutator のローカル変数 |
...
期待 TSV は `(ファイルパス, 行番号)` でデデュープしてある。
analyze.py は同じ行に複数の参照種別（例: 間接 + 間接（setter経由））を出すことがあるが、
KPI 計測の compare() は (filepath, lineno) ベースでマッチングするため、
expected も同じ単位で集約する（最後の行を残す）。
...
4. `(ファイルパス, 行番号)` で重複している場合は dedupe（最後の行を残す）してから expected/ にコピー
