...
from grep_helper.languages.dotnet import (
    classify_usage as classify_usage_dotnet,
...
from grep_helper.languages import dotnet as _dotnet_handler
from grep_helper.file_cache import _file_lines_cache_clear


def _process_grep_file(path, keyword, source_dir, stats):
    return _pgf(path, source_dir, _dotnet_handler, keyword=keyword, stats=stats)
...
class TestClassifyUsageDotnet(unittest.TestCase):
    ...
    def test_CSのstatic_readonlyを定数定義として分類する(self):
        ...
        self.assertEqual(classify_usage_dotnet('public static readonly string ALIAS = STATUS;'), "定数定義(Const/readonly)")

    def test_VBのConst定義を定数定義として分類する(self):
        """VB の Const 宣言が定数定義(Const/readonly)に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('Const STATUS As String = "TARGET"'), "定数定義(Const/readonly)")

    def test_CSの型付き変数代入を変数代入として分類する(self):
        """C# の型指定付き変数代入が変数代入に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('string code = STATUS;'), "変数代入")

    def test_CSのvarキーワード代入を変数代入として分類する(self):
        """C# の var キーワードによる代入が変数代入に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('var x = STATUS;'), "変数代入")

    def test_VBのDim宣言を変数代入として分類する(self):
        """VB の Dim 宣言が変数代入に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('Dim x As String = STATUS'), "変数代入")

    def test_CSのif文を条件判定として分類する(self):
        """C# の if 文が条件判定に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('if (code == STATUS)'), "条件判定")

    def test_VBのIf文を条件判定として分類する(self):
        """VB の If 文が条件判定に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('If code = STATUS Then'), "条件判定")

    def test_CSのreturn文をreturn文として分類する(self):
        """C# の return 文が return 文に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('return STATUS;'), "return文")
    ...
    def test_CSの角括弧Attribute記法を属性として分類する(self):
        ...
        self.assertEqual(classify_usage_dotnet('[Obsolete]'), "属性(Attribute)")

    def test_VBの山括弧Attribute記法を属性として分類する(self):
        """VB の <Attribute> 記法が属性(Attribute)に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('<Serializable>'), "属性(Attribute)")

    def test_メソッド呼び出しの引数をメソッド引数として分類する(self):
        """メソッド呼び出しの引数として渡されたケースがメソッド引数に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('Process(STATUS)'), "メソッド引数")

    def test_どの分類にも該当しない場合はその他になる(self):
        """既知パターンに当てはまらない記述がその他に分類されることを確認"""
        self.assertEqual(classify_usage_dotnet('STATUS'), "その他")
...
class TestExtractConstNameDotnet(unittest.TestCase):
    ...
    def test_CSのpublic_static_readonlyから定数名を抽出する(self):
        ...
        self.assertEqual(extract_const_name_dotnet('public static readonly string ALIAS = STATUS;'), "ALIAS")

    def test_CSのprivate_static_readonlyから定数名を抽出する(self):
        """C# の private static readonly 宣言から定数名 COUNT が抽出されることを確認"""
        self.assertEqual(extract_const_name_dotnet('private static readonly int COUNT = 1;'), "COUNT")

    def test_VBのConst宣言から定数名を抽出する(self):
        """VB の Const 宣言から定数名 STATUS が抽出されることを確認"""
        self.assertEqual(extract_const_name_dotnet('Const STATUS As String = "TARGET"'), "STATUS")

    def test_定義以外の行からはNoneを返す(self):
        """定数定義に該当しない行に対しては None が返されることを確認"""
        self.assertIsNone(extract_const_name_dotnet('string x = STATUS;'))
...
class TestTrackConstDotnet(unittest.TestCase):
    def test_CSとVBの両方で定数の利用箇所を検出する(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            results = track_const_dotnet("STATUS", src, record, stats)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for record in direct_records:
                if record.usage_type == "定数定義(Const/readonly)":
                    ...
                    if const_name:
                        all_records.extend(track_const_dotnet(const_name, src_dir, record, stats))
